...
public class OppWidget_SummaryWidgetTests extends BaseTest {
...
public void widgetRule(HashMap<String, String> testData){
	...
	SObject[] OppId=sfdc.getRecords("select id from Opportunity where AccountId  in (select id from Account where Name='SummaryWidgetTest')");
	OppWidget_SummaryWidgetPage oppWidgetPage = basepage.gotoOpportunityPageWithId(OppId[0].getId()).switchToOppCSWidget().selectSummarySubTab();
	...
	for(String key : itr){
		...
		Assert.assertTrue(oppWidgetPage.verifyWidgetPanel(key, val));
	...
	for(String key : itrr){
		...
		Assert.assertTrue(oppWidgetPage.verifyLeftPanel(key, val));
		System.out.println("Key value is:"+ key+"value is :"+val);  }
	}
...
public void widgetRule2(HashMap<String, String> testData){
	 ...
	 SObject[] OppId=sfdc.getRecords("select id from Opportunity where AccountId  in (select id from Account where Name='SummaryWidgetTest')");
		OppWidget_SummaryWidgetPage oppWidgetPage1 = basepage.gotoOpportunityPageWithId(OppId[0].getId()).switchToOppCSWidget().selectSummarySubTab();
	...
	for(String key : itr){
		String val = widgetPnl.get(key);
		Assert.assertTrue(oppWidgetPage1.verifyWidgetPanel(key, val));
		System.out.println("Key value is:"+ key+"value is :"+val);   }
...
HashMap<String, String> leftPnl = getMapFromData(testData.get("leftWidgetPnl"));
	...
	for(String key : itrr){
		String val = leftPnl.get(key);
		Assert.assertTrue(oppWidgetPage1.verifyLeftPanel(key, val));
		System.out.println("Key value is:"+ key+"value is :"+val);
	}
	}
...
public void widgetRule3(HashMap<String, String> testData){
	sfdc.runApexCode(getNameSpaceResolvedFileContents(DEFAULT_SUMMARY_WIDGET3));
	 ...
	 SObject[] OppId=sfdc.getRecords("select id from Opportunity where AccountId  in (select id from Account where Name='SummaryWidgetTest')");
	OppWidget_SummaryWidgetPage oppWidgetPage2 = basepage.gotoOpportunityPageWithId(OppId[0].getId()).switchToOppCSWidget().selectSummarySubTab();
	
	HashMap<String, String> widgetPnl = getMapFromData(testData.get("widgetPnl"));
	Set<String> itr = widgetPnl.keySet();
	for(String key : itr){
		String val = widgetPnl.get(key);
		Assert.assertTrue(oppWidgetPage2.verifyWidgetPanel(key, val));
		System.out.println("Key value is:"+ key+"value is :"+val);	}
...
HashMap<String, String> leftPnl = getMapFromData(testData.get("leftWidgetPnl"));
	...
	for(String key : itrr){
		String val = leftPnl.get(key);
		Assert.assertTrue(oppWidgetPage2.verifyLeftPanel(key, val));
		System.out.println("Key value is:"+ key+"value is :"+val);
	}
	}
...
public  void editSummary(HashMap<String, String> testData) {
	...
	SObject[] OppId=sfdc.getRecords("select id from Opportunity where AccountId  in (select id from Account where Name='SummaryWidgetTest')");
	OppWidget_SummaryWidgetPage oppWidgetPage3 = basepage.gotoOpportunityPageWithId(OppId[0].getId()).switchToOppCSWidget().selectSummarySubTab();;
	
	HashMap<String, String> edtSumry = getMapFromData(testData.get("edit_Summary_Values"));
	oppWidgetPage3.editSummary(edtSumry.get("Status"), edtSumry.get("Stage"), edtSumry.get("Comments"));
}
