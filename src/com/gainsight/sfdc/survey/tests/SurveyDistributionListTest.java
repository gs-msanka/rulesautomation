...
/*		sfdc.runApexCode(resolveStrNameSpace(CUSTOM_OBJECT_CLEANUP));
    	...
    	JobInfo loadContacts = mapper.readValue(resolveNameSpace(Application.basedir+"/testdata/sfdc/survey/jobs/Job_contacts_into_custom_object.txt"), JobInfo.class);
    ...
    public void testDisrtibutionWithContactObjectAndFilter(Map<String, String> testData) throws IOException {
        ...
        surProp.setStatus("Publish");
        SurveyAddParticipantsPage surveyAddParticipant = surveyPage.clickOnAddParticipants(surProp);
        SurveyAddParticipants surveyParticipants=mapper.readValue(testData.get("AddParticipantsFromContactobjFilter"), SurveyAddParticipants.class); 
        surveyAddParticipant.loadFromContactObject(surveyParticipants);
        surveyAddParticipant.filterConditions(surveyParticipants);
        SurveyDistributePage distribution=surveyPage.clickOnDistribute(surProp);
        distribution.clickingToBeContacted();
        distribution.sendEmail();
       // Assert.assertEquals(getRecordCountFromContactObjectWithFilterCond(), distribution.getContactsCount());
		Assert.assertEquals(
...
/*		SurveyBasePage surveyBasePage = basepage.clickOnSurveyTab();
		...
		SurveyProperties surveyPropData = mapper.readValue(testData.get("PartialAnonymousProperties"), SurveyProperties.class);
		...
		surveyQuestionPage = createSurveyQuestion(surQues, surveyQuestionPage);
		verifyQuestionDisplayed(surveyQuestionPage, surQues);
		SurveyPublishPage publishPage = surveyPage.clickOnPublish(surveyPropData);
		surveySiteURL();
    ...
    public void testDisrtibutionForCompleteAnonymousSurvey(Map<String, String> testData) throws IOException {  	
		SurveyBasePage surBasePage = basepage.clickOnSurveyTab();
		SurveyProperties surveyPropData = mapper.readValue(testData.get("CompleteAnonymousProperties"), SurveyProperties.class);
		surveyPropData.setStartDate(getDateWithFormat(
				Integer.valueOf(surveyPropData.getStartDate()), 0, false));
		surveyPropData.setEndDate(getDateWithFormat(
				Integer.valueOf(surveyPropData.getEndDate()), 0, false));
		...
		publishPage.updatePublishDetails(surveyPropData);
		surveyPropData.setStatus("Publish");
		SurveyAddParticipantsPage surveyAddParticipant = surveyPage.clickOnAddParticipants(surveyPropData);
        SurveyAddParticipants surveyParticipants=mapper.readValue(testData.get("AddParticipant"), SurveyAddParticipants.class); 
        surveyAddParticipant.loadFromContactObject(surveyParticipants);
        surveyAddParticipant.loadParticipants();
        SurveyDistributePage distribution=surveyPage.clickOnDistribute(surveyPropData);
        distribution.clickingToBeContacted();
        distribution.sendEmail();
		Assert.assertEquals(
				getRecordCountFromContactObject(),
