...
public class GSEmailSetup extends BaseTest {
	...
	public void createSurveyWithGSEmail() {
        sfdc.runApexCode(getNameSpaceResolvedFileContents(env.basedir + "/apex_scripts/Surveys/EmailService_NonAnonySurvey.apex"));
        Header hdrs = new Header();
    	SFDCInfo sfinfo = SFDCUtil.fetchSFDCinfo();
    	String endPoint = env.getProperty("ns.appurl");
    	String sessionid = sfinfo.getSessionId();
	...
	public Boolean sendTestEmail() {
	    ...
	    try {
			...
			SObject[] cts = sfdc.getRecords("select id,Name,Email from Contact where AccountId in (select id from Account where Name='Email Test Account')");
			SObject[] sur = sfdc.getRecords(resolveStrNameSpace("Select id from JBCXM__Survey__c where JBCXM__Code__c='EmailServices_Test'"));
	...
	public boolean checkSubAccountInMandrill(String AccessKey,String TenantId,String OrgName) throws Exception {
		...
		ResponseObj httpResp= wa.doPost(uri, hmandrill.getAllHeaders(), rawBody);
		System.out.println("statuscode==" + httpResp.getStatusCode());
