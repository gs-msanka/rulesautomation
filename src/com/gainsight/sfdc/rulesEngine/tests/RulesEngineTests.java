...
public class RulesEngineTests extends BaseTest {
	...
	public void Rule1(HashMap<String, String> testData) throws IOException,
		...
		// Directly giving Advance and trigger criteria as json...instead of
		// constructing the json here.
		// Goto Admin->rules engine and create a rule..then go to object
		// AutomatedAlertRules and get the jsons..and give them in the testdata
		// for first time.
		...
		rSetup.loadUsageDataForRulesEngine(
				getMapFromData(testData.get("UsageData")).get("type"), true , getMapFromData(testData.get("UsageFiles")).get("First"));
		...
		rSetup.loadUsageDataForRulesEngine(
				getMapFromData(testData.get("UsageData")).get("type"), false,getMapFromData(testData.get("UsageFiles")).get("Second"));

		Assert.assertTrue(rValidate.checkAlertsCreated(alertCriteria, alertSeverity[0].getId(),
				alertReason[0].getId(), alertType[0].getId(),alertStatus[0].getId()));
	}
